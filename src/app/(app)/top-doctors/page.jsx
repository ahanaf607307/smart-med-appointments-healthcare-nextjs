'use client';
import TopRatedDoctorsCard from "@/components/TopRatedDoctorsCard/TopRatedDoctorsCard";
...
const page = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['allDoctors'],
    queryFn: async () => {
      const { data } = await axios.get(`/api/allDoctors`);
    ...
    <div className="container mx-auto">
      ...
      <div className=" bg-white px-5 md:px-10 lg:px-4 py-10 lg:min-h-screen gap-5">
        ...
        <div className="text-center">
          <h1 className="text-4xl font-bold uppercase">All <span className="text-[#00BDE0]"> Doctors</span></h1>
        </div>
        {/* End of Heading */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mt-10">
          {
            topRatedMockDoctorsData?.map((doctor) =>
